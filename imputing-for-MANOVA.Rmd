...
is_mout <- column_to_rownames(is_mout, "ID")

is_mout <- is_mout[c(1:12)]

is_mout %>% replace_with_na_all(condition = ~.x > 1)

#is_mout <- rownames_to_column(is_mout, "ID")


#SA3_imputed <- full_join(is_mout, SA3_imputed, by="ID")

#sum(is.na(SA3_imputed))
...
#is_mout <- filter(SA3_imputed, out %in% "multoutlier")
