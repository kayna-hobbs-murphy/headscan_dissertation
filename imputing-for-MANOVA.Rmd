...
imputed_data$race_eth <- as.factor(imputed_data$race_eth)
imputed_data$age_group <- as.factor(imputed_data$age_group)

str(imputed_data)
```

```{r}
SA3_data <- imputed_data[c(1,2,4,5,7,8,12,14,16,17,25,27,28,31,32,33)]
```
...
flextable(combin_out1) %>%
  my_ft_theme()%>% 
  bold(part = "header") %>% 
  set_caption("Univariate and Multivariate Outlier Status: Imputed Data (matches non-imputed)") %>% 
  autofit
#%>% set_header_labels(values = list(TrTr_L = "Alare/AlareCont"))
...
```{r}
#write_xlsx(combin_out1, "C:\\Users\\19177\\OneDrive - Colostate\\Desktop\\Dissertation\\headscan_dissertation\\combin_out1.xlsx")
```


```{r}
##imputing multivariate outliers, all steps below

SA3_imputing <- SA3_data2[c(1:13)]


```

```{r}
#changing multivariate outlier rows to NA
is_mout <- mult_out[c(1, 19)]

SA3_imputing <- full_join(is_mout, SA3_imputing, by="ID")

SA3_imputing1 <- SA3_imputing

which(SA3_imputing1 == "TRUE", arr.ind=TRUE)

SA3_imputing1 <- column_to_rownames(SA3_imputing1, "ID")
  
sum(is.na(SA3_imputing1))

SA3_imputing1[1:23,] <- NA

SA3_imputing1 <- rownames_to_column(SA3_imputing1, "ID")
...
SA3_imputing1 <- SA3_imputing1[-c(2)]
...
Univariate outliers do not seem like outliers based on looking at the data. They are not the max or min of their column.

```{r}
#changing univariate specific measures to NA

is_uout <- combin_out1

is_uout <- filter(is_uout, multv_outlier %in% "Not Outlier")

is_uout <- is_uout[c(1,7)]

SA3_imputing2 <- full_join(is_uout, SA3_imputing1, by="ID")
...
SA3_num <- select_if(SA3_imputing1, is.numeric)

SA3_imputing1 <- rownames_to_column(SA3_imputing1, "ID")


imputed <- imputePCA(SA3_num, ncp=2)
SA3_imputed <- as.data.frame(imputed$completeObs)
SA3_imputed <- round(SA3_imputed, 0)

SA3_imputed <- rownames_to_column(SA3_imputed, "ID")

demographic <- SA3_data2[c(1, 14:16)]


SA3_imputed <- full_join(SA3_imputed, demographic, by="ID")
```



```{r}
#write_xlsx(SA3_imputed, "C:\\Users\\19177\\OneDrive - Colostate\\Desktop\\Dissertation\\headscan_dissertation\\SA3_imputed.xlsx")
```
