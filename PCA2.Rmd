...
```{r}
p <- ggplot(data = PCAdata_full1, aes(x = pc1, y = pc2)) +
  geom_point()
...
```{r}
p <- ggplot(data = PCAdata_full1, aes(x = pc1, y = pc2, color = race_eth, shape = race_eth)) +
...
```{r}
p <- p + stat_ellipse(geom="polygon", aes(fill = race_eth), 
