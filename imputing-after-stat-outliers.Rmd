---
title: "imputing-after-stat-outliers"
output:
...
SA3_noout  <- column_to_rownames(SA3_noout,'ID')

headscan_num <- select_if(SA3_noout, is.numeric)
...
```{r}
quantile_data <- rbind(AA_C_sum, BiW_L_sum, BiW_C_sum, GoSub_C_sum, NRB_L_sum,
                       ProS_L_sum, SelM_L_sum, SnasM_C_sum, TrSman_C_sum, TrTr_C_sum, TrTr_L_sum)
```
...
```{r}
quantile_data <- rbind(AA_C_sum1, BGl_C_sum1, BiW_L_sum1, BiW_C_sum1,
                       GoSub_C_sum1, NRB_L_sum1, ProS_L_sum1, SelM_L_sum1,
                       SnasM_C_sum1, TrSman_C_sum1, TrTr_C_sum1, TrTr_L_sum1)
```
...
```{r}
SA3_noout <- rownames_to_column(SA3_noout, "ID")
imputed_data <- rownames_to_column(imputed_data, "ID")

headscan_demographics <- SA3_noout[c(1,29:33)]
