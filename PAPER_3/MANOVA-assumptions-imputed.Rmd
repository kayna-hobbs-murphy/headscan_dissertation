...
```{r}
SA3_imputed <- read_excel("C:\\Users\\19177\\OneDrive - Colostate\\Desktop\\Dissertation\\headscan_dissertation\\chosen_imputed1.xlsx")
```


```{r}
str(SA3_imputed)
SA3_num <- select_if(SA3_imputed, is.numeric)
```
...
```{r}
variables <- cbind(SA3_imputed$AA_C, SA3_imputed$BiW_C, SA3_imputed$BiW_L, 
                   SA3_imputed$GoSub_C, SA3_imputed$NRB_L, SA3_imputed$ProS_L,
                   SA3_imputed$SelP_L, SA3_imputed$SelM_L, SA3_imputed$SnasM_C,
                   SA3_imputed$TrSman_C, SA3_imputed$TrTr_C, SA3_imputed$TrTr_L)


#part of Dr. Erin Buchanan's lecture, but Manova code returns error: Error in Anova.mlm(mod, ...) : model is singular
#lm_notransf <- lm(variables ~ gender+race_eth+age_group, data=SA3_imputed,
...
manova_out_nt <- manova(variables ~ gender+race_eth+age_group, data=SA3_imputed)
...
boxM( cbind(AA_C, BiW_C, BiW_L, GoSub_C, NRB_L, ProS_L,
           ...
           TrSman_C, TrTr_C, TrTr_L) ~ race_eth, data=SA3_imputed)

boxM( cbind(AA_C, BiW_C, BiW_L, GoSub_C, NRB_L, ProS_L,
           SelP_L, SelM_L, SnasM_C,
           TrSman_C, TrTr_C, TrTr_L) ~ gender, data=SA3_imputed)

boxM( cbind(AA_C, BiW_C, BiW_L, GoSub_C, NRB_L, ProS_L,
           SelP_L, SelM_L, SnasM_C,
           TrSman_C, TrTr_C, TrTr_L) ~ age_group, data=SA3_imputed)

SA3_imputed$gender <- as.factor(SA3_imputed$gender)
SA3_imputed$race_eth <- as.factor(SA3_imputed$race_eth)
SA3_imputed$age_group <- as.factor(SA3_imputed$age_group)

summary(SA3_imputed)

boxtest_data <- SA3_imputed
...
#```{r}
HOMOGENEITY(data=SA3_imputed, groups='race_eth', variables=c('AA_C', 'BiW_C', 
                                                           'BiW_L', 'GoSub_C',
                                                           'NRB_L', 'ProS_L',
                                                           'SelP_L', 'SelM_L', 
                                                           'SnasM_C','TrSman_C', 
...
#```
#```{r}
HOMOGENEITY(data=SA3_imputed, groups='gender', variables=c('AA_C', 'BiW_C', 
                                                           'BiW_L', 'GoSub_C',
                                                           'NRB_L', 'ProS_L',
                                                           'SelP_L', 'SelM_L', 
                                                           'SnasM_C', 'TrSman_C', 
...
#```
#```{r}
HOMOGENEITY(data=SA3_imputed, groups='age_group', variables=c('AA_C', 'BiW_C', 
                                                           'BiW_L', 'GoSub_C',
                                                           'NRB_L', 'ProS_L',
                                                           'SelP_L', 'SelM_L', 
                                                           'SnasM_C', 'TrSman_C', 
...
#```{r}
boxtest_data <- column_to_rownames(SA3_imputed, "ID")
