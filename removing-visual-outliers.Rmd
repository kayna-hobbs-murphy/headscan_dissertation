...
outlier_ids <- subset(headscan_full, GoSub_C>40)
...
headscan_full1 <- headscan_full1 %>% mutate(NRB_L = replace(NRB_L, NRB_L>7, NA))
...
headscan_full1 <- headscan_full1 %>% mutate(SelDH_C = replace(SelDH_C, SelDH_C>4, NA))
...
headscan_full1 <- headscan_full1 %>% mutate(SnasM_C = replace(SnasM_C, SnasM_C>16, NA))
...
headscan_full1 <- headscan_full1 %>% mutate(TrGo_C = replace(TrGo_C, TrGo_C>17, NA))
...
headscan_full1 <- headscan_full1 %>% mutate(TrSman_C = replace(TrSman_C, TrSman_C>29, NA))
...
headscan_full1 <- headscan_full1 %>% mutate(TrSnas_C = replace(TrSnas_C, TrSnas_C>35, NA))
...
```{r}
outl1 <- subset(headscan_full, GoSub_C>40) 
outl1$vis_out <- "GoSub_C"
  
outl2 <- subset(headscan_full, NRB_L>7)
outl2$vis_out <- "NRB_L"

outl3 <- subset(headscan_full, SelDH_C>4)
outl3$vis_out <- "SelDH_C"


outl4 <- subset(headscan_full, SnasM_C>16)
outl4$vis_out <- "SnasM_C"

outl5 <- subset(headscan_full, TrGo_C>17)
outl5$vis_out <- "TrGo_C"

outl6 <- subset(headscan_full, TrSman_C>29)
outl6$vis_out <- "TrSman_C"

outl7 <- subset(headscan_full, TrSnas_C>35)
