...
```{r loading correlation data (previously created in R, has upper data removed)}
...
```{r intraRR iccs wrapped by coder, fig.height=8, fig.width=5}
intraRR %>%
...
```{r low intraRR iccs wrapped by coder, fig.height=8, fig.width=5}
low_intraRR %>%
  mutate(measure=fct_reorder(measure, icc_value, .desc=TRUE)) %>% 
  ggplot(aes(x=icc_value, y=measure))+
    geom_bar(stat= "identity", color= "black", fill = "white")+
...
quest_meas_nona %>% 
  ...
  geom_point(aes(shape=both_measures, color=coder, fill=both_measures), size=5)+
...
```{r intra by cor wrapped plot, fig.height=5, fig.width=8}
quest_meas_nona %>% 
  ggplot(aes(x=intra_icc, y=correlation))+
...
```{r loading inter data and making low inter dataset}
...
```{r joining inter with ques meas data}
...
quest_meas_nona1 %>% 
  ...
  geom_point(aes(shape=both_measures, color=both_measures, fill=both_measures), size=5, na.rm = TRUE)+
  scale_shape_manual(values=seq(0,12))+
  geom_rect(aes(xmin=0.25, xmax=0.75, ymin=0.85, ymax=1), fill= NA, alpha=0.4, colour="black")+
  xlim(0.25, 0.9)+
  theme(text=element_text(family= "Times New Roman"))+
  #theme(axis.text.x = element_text(angle = 90))+ 
...
```{r ploting inter by intra}
quest_meas_nona2 %>% 
  ggplot(aes(x=inter_icc, y=intra_icc, na.rm=TRUE))+
  geom_point(aes(shape=measure, color=measure, fill=measure), size=5, na.rm = TRUE)+
